...
__device__ __host__ static void sha256_transform(SHA256_CTX *ctx, const uint8_t data[]) {
    // CONSTANTS
    // Fractional parts of cube roots of first 64 prime numbers
    // Chosen to introduce "chaos" or "fake randomness"
    // Move k inside function to work with CUDA
    const uint32_t k[64] = {
...
__device__ __host__ void sha256_update(SHA256_CTX *ctx, const uint8_t data[], size_t len) {
...
__device__ __host__ void sha256_final(SHA256_CTX *ctx, uint8_t hash[]) {
    uint32_t i;
    i = ctx->datalen;
...
__device__ __host__ void sha256(const uint8_t *data, size_t len, uint8_t *hash) {
    SHA256_CTX ctx;
    sha256_init(&ctx);
